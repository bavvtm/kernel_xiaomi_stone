...
void rtl8188eus_usb_read_port_complete(struct urb *purb, struct pt_regs *regs)
#else
...
#ifdef __RTL8188EUS_COEXIST_H__
		RTW_INFO("###=> rtl8188eus_usb_read_port_complete => urb.status(%d)\n", purb->status);
#else
...
u32 usb_read_port(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *rmem)
{
	...
	if (precvbuf->pbuf) {
		...
		usb_fill_bulk_urb(purb, pusbd, pipe,
			...
			MAX_RECVBUF_SZ,
#ifdef __RTL8188EUS_COEXIST_H__
			rtl8188eus_usb_read_port_complete,
#else
